...
static void OnTick()
{
	...
	for (Entity entity : entities)
	{
		...
		if (distance < startDistance)
		{
			...
			Memory::ApplyForceToEntity(entity, 3, (entityCoord.x - playerCoord.x) * -3.f,
			                           (entityCoord.y - playerCoord.y) * -3.f, (entityCoord.z - playerCoord.z) * -3.f,
